...
public final class QuickRowsPuzzlePieceSectionLayout: QuickPuzzlePieceSectionLayout, PuzzlePieceSectionLayoutSeperatable {
    ...
    override public func layoutItems(in rect: CGRect, sectionIndex: Int) -> [ItemKey] {
        ...
        for row in 0 ..< numberOfItemsInSection {
            ...
            if rowInfo.intersects(with: rect) {
                itemsInRect.append(ItemKey(indexPath: IndexPath(item: row, section: sectionIndex), kind: nil, category: .cell))
    ...
    override public func shouldCalculatePreferredLayout(for elementCategory: InvalidationElementCategory, withOriginalSize originalSize: CGSize) -> Bool {
        ...
        case .cell(let index):
            shouldInvalidate = (index < numberOfItemsInSection) && rowsInfo[index].heightState == .estimated
